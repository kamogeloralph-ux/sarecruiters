...
// Administration (DPSA) at dpsa.gov.za/newsroom/psvc. Every national and
// provincial government department is legally required to advertise its
// posts through that one circular, so this single source already covers all
// national + provincial public-service vacancies -- it isn't a "DPSA-only"
// subset of a bigger government job market, DPSA IS the aggregator for all
// of them. What it does NOT cover: local government/municipalities (each
// advertises independently, no central circular) and state-owned entities
// like Eskom, Transnet, SAA, PRASA, SARS, SAPS etc. (each has its own
// careers site). Those would need separate scrapers if wanted.
const execFileAsync = promisify(execFile);
const __dirname = path.dirname(fileURLToPath(import.meta.url));
const DPSA_BASE = 'https://www.dpsa.gov.za'; // literal source URL -- the actual site this scrapes, not a naming leftover
const DPSA_PAGE_BASE = `${DPSA_BASE}/newsroom/psvc`;
const GOVERNMENT_TIMEOUT_MS = Number(process.env.GOVERNMENT_TIMEOUT_MS || 30_000);
const GOVERNMENT_MAX_CIRCULAR = Number(process.env.GOVERNMENT_MAX_CIRCULAR || 60);
const GOVERNMENT_YEARS = String(process.env.GOVERNMENT_YEARS || new Date().getUTCFullYear())
